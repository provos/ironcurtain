...
Owns the long-running model, serialization, the in-use interlock, the operation-record store, and security validation.
...
`startCompile(name, actor, eventBus, allowPolicyMutation, isPersonaInUse)`:
1. If `!allowPolicyMutation` ⇒ `POLICY_MUTATION_FORBIDDEN` (the dispatch checks this **before** any credential read, so a client without mutation enabled never learns credential state).
1a. **In-use interlock:** if `isPersonaInUse(name)` ⇒ `PERSONA_IN_USE`. `isPersonaInUse` is a predicate **injected by the daemon** (which holds both the `WorkflowOrchestrator` and this orchestrator in scope), implemented as `workflowOrchestrator.isPersonaActive(name)` over in-flight workflow instances — keeping `src/persona/` free of any `src/workflow/` import. This closes the hot-swap path (§7): a persona referenced by a running workflow cannot be recompiled (operator recompiles after the run finishes). Single CLI/daemon sessions are unaffected — they load policy once at construction and never re-read it; **only** workflow shared-container mode re-reads per state entry, so only it needs the interlock.
...
// errors: PERSONA_IN_USE (referenced by an in-flight workflow), COMPILE_IN_PROGRESS (live operationId in data),
//         COMPILE_QUEUE_FULL, CREDENTIALS_MISSING, BROAD_POLICY_REJECTED (terminal, via failed event),
//         LIST_REQUIRES_MCP (terminal), POLICY_MUTATION_FORBIDDEN
...
`PERSONA_EXISTS`, `PERSONA_IN_USE`, `COMPILE_IN_PROGRESS`, `COMPILE_QUEUE_FULL`, `CREDENTIALS_MISSING`, `POLICY_MUTATION_FORBIDDEN`, `LIST_REQUIRES_MCP`, `BROAD_POLICY_REJECTED`.
...
- **In-use interlock** (replaces the heavier "snapshot" approach): `compileStream` is **refused with `PERSONA_IN_USE`** when an in-flight workflow references the persona, via the daemon-injected `isPersonaInUse` predicate (§4.3). Because mid-session policy hot-swap happens **only** in workflow shared-container mode (`cyclePolicy` re-reads the live dir per agent-state entry; single sessions load once at construction and never re-read), refusing the recompile while the persona is busy fully closes the hot-swap-into-a-running-run path — with **zero** change to the workflow engine's policy resolution (`getPolicyDir`/`cyclePolicy` untouched). The existing server-set-expansion guard (`orchestrator.ts:1125-1138`) stays as harmless defense-in-depth. (A copy-at-start *snapshot* — recompile freely, applies next run — is a possible Phase-2 generalization, not a v1 necessity.)
...
- **Prompt-injection ceiling** — the attacker controls the constitution text fed to the compiler LLM. Two structural controls **independent** of the LLM:
  ...
  2. **Compile-time diff** — the `done` event and `PersonaCompileResultDto.result` carry `ruleDelta` vs the persona's previous `compiled-policy.json`; the UI surfaces it. Because Phase-1 compile writes only the persona dir and the in-use interlock keeps in-flight runs from recompiling, the diff is reviewable **before** the policy governs any new session, so no blocking confirm-before-write is needed.
...
| **1b** | ctx-first `personaDispatch` (+`client`/`connId`); `event-bus-progress-reporter.ts` + `persona-compile-orchestrator.ts` (active Map + recent LRU + FS lock + `globalLimit` + wall-clock cap + stale-lock recovery + snapshot-as-truth); thread opts through `compilePersonaPolicy` (incl. persona-workspace `allowedDirectory` + `allowMcpLists:false`); `personas.compileStream` + the 5 events + `getCompile`/`listCompiles`; `CREDENTIALS_MISSING` preflight (both models) + post-dispatch classification. **In-use interlock** in the orchestrator (`isPersonaInUse` predicate injected by the daemon from `workflowOrchestrator.isPersonaActive` ⇒ `PERSONA_IN_USE`) — the containment mechanism, ships here. Frontend live indicator + reconnect hydrate. Mock + e2e. `personas.compile` kept blocking (no breaking change). | Yes (compile streaming) | ~3–4 d |
...
|---|---|---|---|---|
| 1 | In-flight workflow picks up a WS-weakened persona policy (no rule-weakening guard) | critical | **fixed** | **In-use interlock**: `compileStream` refused (`PERSONA_IN_USE`) while a workflow references the persona. Hot-swap is workflow-shared-container-mode-only, so this closes it without touching `getPolicyDir`/`cyclePolicy` (§4.3, §7). Snapshot approach dropped as over-built for v1. |
...
| 21 | Phase 2 global compile writes the live active dir — breaks the "never write active dir" guarantee | medium | **deferred** | Sharpened: dir-swap mandatory for global; `GLOBAL` singleton key; stronger operator confirmation; global policy governs all sessions, so Phase 2 is where a copy-at-start snapshot likely earns its cost (the interlock alone is insufficient there) (§10). |
...
- **Recompile blocked while a persona is in use by a workflow** (`PERSONA_IN_USE`) — the operator recompiles after the run finishes. The "recompile anytime, applies next run" snapshot is a deferred Phase-2 nicety.
