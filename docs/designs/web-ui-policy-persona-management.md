...
Because policy *is* the security boundary, exposing mutation to a WS client is a privilege-escalation surface: a token-bearing (or token-hijacked) browser tab could compile an allow-all constitution. The design therefore lands four structural defenses **before** any mutation method ships: (1) a two-layer authz gate (a default-off `--allow-policy-mutation` kill switch **and** a separate admin capability token not present in the printed URL); (2) a directory-atomic artifact swap that closes a measured ~97%-corruption torn-read race against the runtime policy loader; (3) a post-compile structural broad-policy validator that rejects `*`-domain / out-of-workspace policies regardless of what the LLM produced (defeating constitution prompt injection), plus disabling data-backed (MCP) dynamic-list resolution on the WS path; and (4) a workflow **policy snapshot** so a WS recompile of an in-flight persona cannot be hot-swapped into a running session — it takes effect only on the next workflow start.

Global (non-persona) policy compilation and `annotate-tools` are deferred (Phases 2–3) precisely because they write the *live* active policy dir and/or require in-process MCP connections the daemon must not own.
...
- `annotate-tools` over WS → **Phase 3** (forked supervised subprocess; never in-process — see §10 A5).
...
- Multi-user / non-localhost daemon hardening of the connection token (deferred; the admin capability removes mutation from the connection token's blast radius — tracked prerequisite, §12).
...
/** actor = `${remoteAddr}#${connId}` + admin-capability id (WS), or 'cli'/'cron'. */
...
Owns the long-running model, serialization, snapshot store, and security validation.
...
`startCompile(name, actor, eventBus, allowPolicyMutation, hasAdminCap)`:
1. If `!allowPolicyMutation || !hasAdminCap` ⇒ `POLICY_MUTATION_FORBIDDEN` (the dispatch checks this **before** any credential read so a non-admin client never learns credential state).
...
5. If `globalLimit` is saturated and queue depth would exceed ~10 ⇒ `COMPILE_QUEUE_FULL`; else enqueue (return `{ accepted, operationId, queued }`). Inside `globalLimit`, run the detached promise:
   ...
   - `await compilePersonaPolicy(name, { reporterFactory: (s) => new EventBusProgressReporter({...}), signal, quiet: true, operationId, allowMcpLists: false, allowedDirectory: getPersonaWorkspaceDir(name), stagingDir })` — compiles into a **staging dir** `generated.tmp-<uuid>/`.
   - Run the **broad-policy validator** (§7) on the staged `compiled-policy.json`: reject if any `domains.allowed` contains `'*'`, any `lists[].allowed` contains `'*'`, or any `paths.within` resolves outside `getPersonaWorkspaceDir(name)` — **unless** `persona.allowBroadPolicy === true`. Reject ⇒ `BROAD_POLICY_REJECTED`, staging dir discarded.
   - Compute `ruleDelta` vs the persona's **previous** `compiled-policy.json`.
   - **Promote** staging → `generated/` via atomic **directory** `renameSync`, **only if** `active.get(name)?.operationId === operationId` (a settled-late zombie cannot clobber a newer compile).
...
On **daemon startup**: scan persona dirs for `.compile.lock`; for stale locks (dead pid or past cap) remove the orphaned `generated.tmp-*` staging dir(s), emit a synthetic `persona.compile.failed` (operationId from the lock) so a reconnecting UI clears the stuck card, and release the lock. Live (pid-alive) locks have their wall-clock cap re-armed.
...
readonly stagingDir?: string;     // when set, all artifacts write here; caller owns the dir-rename promotion
...
- **Directory-atomic swap** for the multi-file artifact set (supersedes per-file atomicity there): the runner writes the full set into `generated.tmp-<uuid>/`; the orchestrator validates then `renameSync`s the directory into place. `loadPersonaPolicyArtifacts` (`config/index.ts:428-429`) always sees a mutually consistent set, including the empty-lists case.
...
```
Assign a `connId` at `handleUpgrade` (`web-ui-server.ts:451`) stored in a `WeakMap<WsWebSocket, string>`; `actor = `${remoteAddr}#${connId}`` + the admin-capability id (§7). Bind the admin capability to the connection at upgrade (not per-frame).

Add to `MethodName` (`web-ui-types.ts:21`) and `ErrorCode` (`:88`).

### New / changed methods (all params via `validateParams`; mutation methods require `allowPolicyMutation` **and** the admin capability, else `POLICY_MUTATION_FORBIDDEN`; read methods ungated)
...
// personas.compile  (KEPT BLOCKING, back-compat, deprecated-but-supported) -> PersonaCompileResultDto
...
- **Cancellation (v1, honest):** the wall-clock cap (20 min) fires an `AbortSignal` that aborts at the next phase boundary of each in-flight server **and** cancels in-flight `generateText` HTTP calls. `operationId` is the reserved handle for a future `personas.cancelCompile`. The directory-swap-on-current-op-only check makes a settled-late zombie unable to clobber a newer compile, and abandoned per-server artifacts (under the staging dir) cannot poison the content-hash cache.
- **Restart recovery:** the FS lock is the in-progress sentinel; the staging-dir + atomic promotion means the live `generated/` is never left mid-flight. The startup stale-lock scan emits synthetic `persona.compile.failed` so reconnecting UIs clear stuck cards. Atomic artifacts make recompile idempotent.

---

## 7. Security, authz & audit model

Policy *is* the boundary, so mutation gets defense beyond the single bearer token.

### Threat
With no per-method authz, any token-bearing/hijacked tab could compile an allow-all constitution that — in workflow shared-container mode — gets hot-swapped live via `ToolCallCoordinator.loadPolicy` (`tool-call-coordinator.ts:551`). The connection token is in the printed `?token=` URL (`web-ui-server.ts:168`), logged by proxies/history.

### Two-layer authz
1. **Kill switch — `--allow-policy-mutation`** (default **OFF**, CLI-only per invocation; **not** config-persisted, so it can't be silently enabled by editing `config.json`). 4-hop plumbing mirroring `captureTracesDefault`: `daemon-command.ts` CommandSpec option (`:39` block) + `parseArgs` option (`:92` block) + read + pass → `IronCurtainDaemonOptions` field → `WebUiServerOptions` field → `dispatchCtx` (`web-ui-server.ts:96`). When off, **all** mutation methods return `POLICY_MUTATION_FORBIDDEN` regardless of token, and `DaemonStatusDto.allowPolicyMutation` (populated in `buildStatusDto`, `dispatch/types.ts:94`, from `ctx` — the field is added to `DispatchContext`) lets the UI hide controls.
2. **Admin capability** — a **separate** admin token, **not** in the auto-printed `?token=` URL, retrieved via a new `ironcurtain daemon admin-token` command, bound to the connection at `handleUpgrade`, verified with `timingSafeEqual`. The four mutation methods + `setBroadPolicyOptIn` + `compileStream` require **both** the connection token **and** the admin capability. Read methods stay ungated.
...
- **Workflow snapshot** (the cornerstone): at workflow **start**, copy each referenced persona's `generated/` dir (including `GLOBAL_PERSONA → getUserGeneratedDir()`) into the per-run tree `~/.ironcurtain/workflow-runs/<id>/policy-snapshots/<persona>/` via an atomic staging+dir-rename copy. `getPolicyDir` (`orchestrator.ts:1061`) resolves to the **snapshot**, never the live dir; `cyclePolicy` loads only from the immutable snapshot. A WS/global recompile therefore takes effect only on the **next** workflow start — it cannot alter an in-flight run. The existing server-set-expansion guard (`orchestrator.ts:1125-1138`) stays as harmless defense-in-depth.
- **Path traversal** closed by `createPersonaName` branding before any fs op.
- **Atomicity** (directory-swap + FS lock) closes the corruption/over-permit races.
...
|---|---|---|---|
| **0** | `atomicWriteTextSync`; directory-atomic-swap in the runner (`stagingDir` + orchestrator-owned dir-rename); `reporterFactory`/`signal`/`quiet`/`allowMcpLists`/`stagingDir` on `PipelineRunConfig` + the two reporter substitutions + per-phase signal checks + `abortSignal` through `generateText`; per-`operationId` LLM log; ~24 `console.error` suppression-when-quiet; ESLint `no-restricted-imports` boundary rule + madge static-graph test. | No | ~2–3 d |
...
| **1b** | ctx-first `personaDispatch` (+`client`/`connId`); `event-bus-progress-reporter.ts` + `persona-compile-orchestrator.ts` (active Map + recent LRU + FS lock + `globalLimit` + wall-clock cap + stale-lock recovery + snapshot-as-truth); thread opts through `compilePersonaPolicy` (incl. persona-workspace `allowedDirectory` + `allowMcpLists:false`); `personas.compileStream` + the 5 events + `getCompile`/`listCompiles`; `CREDENTIALS_MISSING` preflight (both models) + post-dispatch classification. **Workflow snapshot** in the orchestrator (ships here — the containment cornerstone). Frontend live indicator + reconnect hydrate. Mock + e2e. `personas.compile` kept blocking (no breaking change). | Yes (compile streaming) | ~4–5 d |
| **1c** | `personas.create`/`editConstitution`/`setMemory`/`delete`/`setBroadPolicyOptIn` + DTOs + new error codes + `personas.changed`; `--allow-policy-mutation` kill switch (4-hop) + admin-token capability + `connId` + persona-service audit (chained); broad-policy validator + `ruleDelta`; soft-delete → `.persona-trash`. Frontend create form / edit textarea / delete confirm / memory toggle / `ruleDelta` / mutation-gating. | Yes (full CRUD) | ~4–5 d |
| **2** (deferred) | `compileGlobalPolicy(opts)` packaged like `compile-persona-policy.ts` (writes `getUserGeneratedDir()` = the **live active dir**, `config/paths.ts:284`), reached by `await import`; `policy.*` dispatch reusing fire-and-return + events + orchestrator. **Hard prereqs:** directory-atomic-swap is **mandatory** for the global dir; a reserved `GLOBAL` singleton serialization key; stricter admin confirm (daemon-wide blast radius). The workflow snapshot already covers `GLOBAL_PERSONA`. Security model restated: "persona-dir-only writes" is a **Phase-1-only** property. | Yes (global) | (future) |
| **3** (deferred, optional) | Extract the **pure** customizer core (`applyChanges`+diff+`callLlm`) into a headless step shared by the `@clack` loop and a WS handler (prevents divergence); customize over WS (client holds `ModelMessage[]` per `operationId`); `annotate-tools` as a daemon-**supervised forked subprocess** — rationale: orphan risk (A5) **and** `annotate.ts:256`'s global `process.env.ALLOWED_DIRECTORY = process.cwd()` mutation would pollute every in-daemon compile, so it must never run in-process. | Yes (authoring) | (future) |
...
- orchestrator: `COMPILE_IN_PROGRESS` dedup; FS-lock cross-process exclusion; `active → recent` synchronous transition + a second compile fired in the settle-but-not-cleaned window; stale-lock startup recovery (dead-pid + past-cap); `globalLimit` queueing + `COMPILE_QUEUE_FULL`; settled-late zombie cannot promote.
- broad-policy validator: rejects `'*'`-domain / `'*'`-list / out-of-workspace `paths.within`; passes when `allowBroadPolicy`.
- per-op LLM log non-truncation across two sequential compiles.
- **A4 concurrency:** a worker-thread writer doing the full artifact **set** + directory swap vs the **actual** two-read `loadPersonaPolicyArtifacts` loop (`compiled-policy.json` + `dynamic-lists.json`), asserting the pair is always mutually consistent **including the empty-lists case**; plus a `constitution.md` **text** path (`atomicWriteTextSync`) and a `persona.json` path (`setPersonaMemory` writer vs `scanPersonas`/`loadPersona` reader).
...
| **A4** | `writeArtifact` is non-atomic; partial reads observable; needs atomic + multi-file fix. | **CONFIRMED + ESCALATED** | `writeArtifact = mkdirSync+writeFileSync` (`pipeline-shared.ts:195-198`); worker-thread reader saw ~97% partial-read corruption; per-file atomicity is **insufficient** — `loadPersonaPolicyArtifacts` reads two files non-transactionally (`config/index.ts:428-429`), with an empty-lists SKIP (`pipeline-runner.ts:657`) leaving a permanent stale file ⇒ **directory-atomic swap**. `atomicWriteJsonSync` always stringifies (`escalation-watcher.ts:51-55`) ⇒ new `atomicWriteTextSync` for markdown. |
| **A5** | `annotate-tools` needs in-process MCP connections the daemon must not own; fork it. | **CONFIRMED** | `annotate.ts` spawns raw `StdioClientTransport` per server held across LLM annotation; `compile-policy` uses only `connectViaProxy` (one supervised child); the daemon has zero MCP connections. **Stronger** reason added: `annotate.ts:256` does a global `process.env.ALLOWED_DIRECTORY = process.cwd()` mutation that would pollute every in-daemon compile ⇒ fork in Phase 3. |
...
| **A6** | Single bearer token is insufficient authz for mutation. | **DESIGN DECISION (not a code spike)** | Threat-modeled ⇒ two-layer gate (kill switch + admin capability) + persona-service audit + blast-radius containment (snapshot/atomic/broad-policy validator). |
| **(workflow hot-swap)** | A WS recompile of an in-flight persona is hot-swapped into a running session. | **CONFIRMED (devil's-advocate)** | `getPolicyDir` resolves the **live** persona dir (`orchestrator.ts:1064`, `:4542`); `cyclePolicy` (`:1111`) loads from it; the only guard rejects server expansion (`:1125-1138`). ⇒ **workflow policy snapshot** fix (the cornerstone). (The *separate* containment-governance question is resolved by A9 below.) |
...
|---|---|---|---|---|
| 1 | In-flight workflow picks up a WS-weakened persona policy (no rule-weakening guard) | critical | **fixed** | Workflow policy **snapshot** into the per-run tree; `getPolicyDir` resolves the immutable snapshot; recompile applies only on next start (§7). |
| 2 | `--allow-policy-mutation` is a process-global boolean gating a per-request capability | critical | **mitigated** | Two-layer: kill switch + separate admin token bound at upgrade (§7). Connection-token-out-of-URL deferred (admin token removes mutation from its blast radius). |
| 3 | Atomic per-file write doesn't close the cross-file torn read (`compiled-policy.json` + `dynamic-lists.json` 34 lines apart; empty-lists SKIP = permanent stale) | critical | **fixed** | **Directory-atomic swap**: staging dir + orchestrator dir-rename after validation; empty-lists case omits the file (§4.5). |
...
| 5 | Attacker-controlled constitution → over-permissive rules (prompt injection) | high | **fixed** | LLM-independent broad-policy validator (rejects `'*'`/out-of-workspace unless `allowBroadPolicy` opt-in) + compile-time `ruleDelta`; doc that LLM-judge ≠ authz (§7). |
...
| 11 | Daemon restart mid-compile orphans the op; `generated/` indeterminate | high | **fixed** | FS lock sentinel + staging-dir + atomic promote ⇒ live dir never mid-flight; startup stale-lock scan emits synthetic `failed` (§4.3, §6). |
...
| 21 | Phase 2 global compile writes the live active dir — breaks the "never write active dir" guarantee | medium | **deferred** | Sharpened: dir-swap mandatory for global; `GLOBAL` singleton key; stricter admin confirm; snapshot covers `GLOBAL_PERSONA` (§10). |
...
| 25 | Best-effort cancel poisons the per-server content-hash cache | medium | **fixed** | Staging-dir per-op artifacts promoted only on success; abort discards staging (§4.3). |
...
| 27 | No per-LLM-call timeout; wall-clock cap can't abort an in-flight call | medium | **fixed** | `abortSignal` through `createThrottledModel`/`generateText` + promote-only-if-current-op guard (§4.4, §6). |
...
2. **Connection-token-in-URL hardening** (`web-ui-server.ts:168`): move the connection token out of the query string (cookie / `Sec-WebSocket-Protocol`) before exposing to any multi-user / non-localhost daemon. Tracked prerequisite; the admin token removes mutation from its blast radius for single-operator localhost (the v1 target). Decide owner + phase.
3. **Audit-chain secret durability**: per-daemon-lifetime tamper-evidence (in-memory secret) vs cross-restart continuity (persisted secret = its own key-management problem). Defaulting to per-lifetime; confirm.
4. **`globalLimit` value** (default 2) and queue depth cap (~10): validate against operator usage + provider rate limits; decide if config-tunable.
...
- **Coarse admin capability** — single admin token (documented upgrade: per-actor capabilities).
