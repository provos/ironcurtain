...
**As shipped, the `preflight` prompt corrects this in the spec it authors** (`workflow.yaml`, the `evolve-brief normalize` step — search "add 1 to MAX*ROUNDS"): it adds **+1 to `--max-rounds` only when `/workspace/initial_program` exists** (i.e. only when node 0 is a seeded baseline). The prompt is explicit: "The seed is copied verbatim as node 0 and the engine counts every recorded node toward done_rounds, so without the +1 a request for N rounds would yield only N−1 evolution rounds (the seed consumes one). The +1 restores the requested number of evolution rounds. Do NOT add the +1 when no initial_program is seeded (node 0 is then a real first round)." From-scratch runs (no seed) are unchanged — node 0 there \_is* a genuine first evolution round, so no adjustment is made. This is a real correction to the round-budget semantics introduced by seeding, contained entirely in the `preflight` prompt (no engine or bridge change).
