...
Status: **Implemented & merged in #309 (2026-06-17).** `evolve` is now problem-agnostic: `--workspace <experiment-dir>` provisions deps into the persistent venv, infers the run spec from the experiment's own files, scores candidates via the experiment's real evaluator, and reaches `done` — with zero per-experiment edits to `workflow.yaml`/`scripts/`/`requirements.txt`.

> **As-shipped record.** The sections below are the original design narrative, preserved, with as-shipped corrections and the post-design refinements that emerged during implementation captured inline (flagged **AS SHIPPED:**). The slice landed substantially as designed; the notable deltas the implementation surfaced — an experiment-aware **evaluator timeout** (the design assumed a fixed budget, the shipped `evaluate` path needs a generous one), a **task-derived success criterion**, and a **seed-aware `--max-rounds +1`** correction (a round-budget off-by-one not anticipated in the design) — are documented where the relevant design section sits. Verified against the merged tree: `src/workflow/workflows/evolve/workflow.yaml`, `src/workflow/workflows/evolve/scripts/evolve_result.py` and the vendored `evolve_core`, `src/workflow/daemon-gate-commands.ts`, and `test/workflow/evolve-experiment-harness.integration.test.ts`.
>
> Builds ON the merged human-surface FSM (`docs/designs/evolve-human-surface-slice.md`: `preflight_review` / `human_escalation` / `final_summary` / `final_review` gates + the `aborted` terminal). The two maintainer decisions (§13) — (1) initial-program seeding from the experiment dir, (2) the acceptance bar — shipped **IMPLEMENTED as decided** at their recommended defaults. The lower-stakes implementer-choice items (§13) were resolved as the shipped code shows (e.g. `provision blocked → provision_review`).
...
`circle_packing_demo/input.md:1-40`. A human-readable problem statement: "Pack 26 circles in a unit square (1×1) to maximize the sum of their radii" (`:3-5`), the objective "Maximize: Σ(r_i)" (`:7-9`), constraints (inside the unit square, non-overlapping, `:11-14`), the target "sum of radii = **2.635**" (`:16-18`), and the required entry point — a `construct_packing()` returning `(centers, radii, sum_radii)` (`:22-33`). **`preflight` infers from this**: the `run_spec.objective` (one line), the `evaluation.success_criteria`, and the candidate's interface contract (the `construct_packing()` signature) for the cognition seed / initial program.

> **AS SHIPPED (success criterion — task-derived, not a fixed placeholder).** The shipped `preflight` prompt (`workflow.yaml`, the `SUCCESS_CRITERION` step) derives the criterion **from the task's stated goal first**: when the task names a benchmark (e.g. "toward the AlphaEvolve target"), `preflight` resolves the concrete number from the experiment brief and uses it (e.g. `eval_score >= 2.635`), falling back to the brief's own stated target, and only to a generic `eval_score >= 1.0` when neither states one. It is explicitly a **recorded quality descriptor, not a hard gate** — the run stops on `max_rounds`, not on this criterion — so the prompt prefers an accurate target over a conservative placeholder. The design's "fixed `eval_score >= 2.0` bar; 2.635 aspirational" wording in §2.1/§7.1/§7.3 is therefore superseded: the criterion is the resolved benchmark target where one exists. Reaching that target remains aspirational (it gates nothing); only the criterion _string_ is task-derived.
...
> **AS SHIPPED (minor — daemon `--workspace` validation hardening).** The client-side `--workspace` check in `daemon-gate-commands.ts` (`:319-334`) wraps the `statSync(workspacePath).isDirectory()` probe in `try/catch`: any stat failure (ENOENT — including a racy delete after a check — EACCES, ELOOP, …) is treated as "not a directory" and returns the structured `WORKSPACE_NOT_DIRECTORY` error instead of letting the exception crash the CLI. The check runs _before_ connecting to the daemon, so an invalid `--workspace` fails fast without spawning or connecting.
...
- **Success criterion** ← **AS SHIPPED**, the task's stated goal first (resolving a named benchmark to its concrete number, e.g. `--success-criterion "eval_score >= 2.635"`), falling back to the brief's target, and to a generic `eval_score >= 1.0` only when neither states one (§2.1 note). It is a recorded descriptor, not a gate — the run stops on `max_rounds`. `success_criteria` must be non-empty (`run_state.py:71-73`).
- **Evaluator timeout** ← **AS SHIPPED**, a generous experiment-aware budget → `--evaluation-timeout-secs` defaulting to ~300s, raised toward ~600 for heavy numerical evaluation (§7.3 note). Not in the original design's inference list; added because the live `evaluate` path needs more than the sketched 30s.
...
- **Budget / sampler** ← the task string's requested round count (default 3) and the package defaults → `--max-rounds N --sampling-algorithm greedy --sample-n 1` (unchanged from today's prompt). **AS SHIPPED:** when `/workspace/initial_program` is seeded, `preflight` passes `--max-rounds N+1` so the seeded node 0 does not consume an evolution round (§7.4.1).
...
evaluation:
  ...
  timeout_secs: 300 # AS SHIPPED — experiment-aware, ~300 default (see note); was sketched as 30
  success_criteria: ['eval_score >= 2.635'] # AS SHIPPED — task-derived target (§2.1 note); was sketched as `>= 2.0`
cognition:
  source_mode: seed
...
> **AS SHIPPED (evaluator timeout — experiment-aware, NOT a hardcoded 30).** The design's `timeout_secs: 30` was an under-estimate that the live run exposed: 30s truncates slow-but-valid candidates (e.g. SLSQP-heavy circle-packing) as failures. The shipped harness handles this entirely in the **`preflight` prompt** (the vendored engine and the `evolve_result.py` bridge are untouched): `preflight` authors `--evaluation-timeout-secs EVAL_TIMEOUT_SECS` with a **generous default of ~300s**, raised toward ~600 when the experiment/task implies heavy numerical evaluation or declares a larger per-candidate budget (`workflow.yaml`, the `EVAL_TIMEOUT_SECS` inference step). `evolve-brief normalize` writes that into `run_spec.evaluation.timeout_secs` (`evolve_core/cli.py:145-146`). At score time the bridge's `evaluate` subcommand forwards `--timeout` to `evolve-eval` **only when explicitly given** (`evolve_result.py:190-191`); the `evaluate` state's `run:` command does not pass one, so the engine falls back to `spec.evaluation.timeout_secs` (`evolve_core/cli.py:264-266`). Net: the per-candidate budget is the run-spec value `preflight` authored, not a fixed 30. The synthetic CI fixture (§11.1) still writes `timeout_secs: 30` in its mock-authored spec — its trivial evaluator finishes instantly, so the generous-timeout behavior is exercised only by the real `preflight` prompt on the live run, not in CI.
...
**RESOLVED (was an open question):** copying `initial_program` into `steps/<step>/code` is within `mutation_scope` (it writes under `.evolve_runs`, the writable path) and the `researcher` reads `/workspace/initial_program` from the workspace root. Both confirmed at build time. The shipped `researcher` prompt (`workflow.yaml`) carries the round-1 clause: "if parent is null and `/workspace/initial_program` exists, copy it verbatim as this round's candidate without modifying it — it is the experiment's baseline, and the evaluator will score it as node 0."

#### 7.4.1 Seed-aware `--max-rounds +1` (AS SHIPPED — NOT in the original design)

A round-budget off-by-one surfaced on a live run that the design did not anticipate. Because decision 1 seeds `initial_program` as **node 0**, and the engine counts **every recorded node** toward `done_rounds` (`done_rounds = node count`, per `evolve_result.py:_node_count` / the orchestrator's "count its nodes map"), a request for N rounds would yield only **N−1 evolution rounds** — the seed consumes one of the N. The seed is supposed to be the _baseline the search improves on_, not one of the search rounds, so this silently shortchanges the round budget.

**As shipped, the `preflight` prompt corrects this in the spec it authors** (`workflow.yaml`, the `evolve-brief normalize` step — search "add 1 to `MAX_ROUNDS`"): it adds **+1 to `--max-rounds` only when `/workspace/initial_program` exists** (i.e. only when node 0 is a seeded baseline). The prompt is explicit: "The seed is copied verbatim as node 0 and the engine counts every recorded node toward done_rounds, so without the +1 a request for N rounds would yield only N−1 evolution rounds (the seed consumes one). The +1 restores the requested number of evolution rounds. Do NOT add the +1 when no initial_program is seeded (node 0 is then a real first round)." From-scratch runs (no seed) are unchanged — node 0 there _is_ a genuine first evolution round, so no adjustment is made. This is a real correction to the round-budget semantics introduced by seeding, contained entirely in the `preflight` prompt (no engine or bridge change).
...
> **AS SHIPPED — gate met (green).** All three conditions below hold in #309. (a) The synthetic-fixture integration test shipped as `test/workflow/evolve-experiment-harness.integration.test.ts` — it stages a synthetic `--workspace` (a one-line `requirements.txt: tabulate`, a real `import tabulate` evaluator, an `initial_program` seed), drives a real `uv pip install` of `tabulate` through the registry proxy via the `provision` mock branch, asserts the `.provisioned` marker, that `import tabulate` succeeds in the venv, that the PyPI install was audited (`countTabulateAudit > 0`) and that the loop adds **zero** further `tabulate` installs (provision-once), real-evaluator node scores, lineage, and reaching `done` — plus the no-experiment toy-run regression case. (b) `workflow lint … --strict` is clean with `provision` + `provision_review` + `initial: provision`. (c) The live circle-packing run reached `done`, improving best from **0.96 → 2.6173** with **zero timeout truncations** (the generous EVAL_TIMEOUT_SECS, §7.3) — provisioning scipy, scoring via the real `evaluator.py`, best improving. Reaching the 2.635 AlphaEvolve target stays aspirational, not gating.
...
### Implementer-choice items — RESOLVED as shipped

> These were the lower-stakes implementer-choice items; each is annotated below with what actually shipped in #309. The earlier `--experiment`-threading open items (the `--experiment`-on-`workflow run` question, the `start()` signature question, the mount-from-source-vs-copy question) were already **GONE** — the slice reuses the existing `--workspace`/`workspacePath` plumbing, threaded end-to-end (CLI → RPC → `controller.start`).
...
- **`provision blocked → provision_review` (gate) vs `→ failed` (hard-fail) (§4.2) — SHIPPED: the gate.** The shipped `workflow.yaml` routes `provision ─blocked→ provision_review` (a `human_gate` with `acceptedEvents: [APPROVE, FORCE_REVISION, ABORT]`, `APPROVE`/`FORCE_REVISION` looping back to `provision` with `resetVisitCounts: [provision]`, `ABORT → aborted`), exactly the recommended option — for symmetry with the human-surface slice's "human-actionable stops get a gate" principle. The `→ failed` alternative was not taken.
- **Resume + provision skip (§6.2, §8).** On `workflow resume`, the same `--workspace` experiment dir is re-mounted at `/workspace` by the existing workspace-mount machinery (the workspace path persists and is re-resolved on resume), and the shipped `provision` prompt (`workflow.yaml`, step 1) skips the durable-venv install via the `/workspace/.evolve_runs/main/.provisioned` marker after re-verifying the recorded packages import; apt is re-applied unconditionally (§6.2).
