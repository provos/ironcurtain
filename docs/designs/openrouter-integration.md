...
**Decision.**
- **Codex.** `createCodexAdapter(userConfig)` (signature changed, m3). `generateMcpConfig()` writes `codex-config.toml` with root keys FIRST (`model = <codexSlug>`, `model_provider = "openrouter"`) then the `[model_providers.openrouter]` table (§4.6/B1). **Codex slug = `perAgent.codex ?? resolveMappedModel(DEFAULT_GLM_SLUG, modelMap) ?? DEFAULT_GLM_SLUG`** (no native model field → never passthrough an unmapped OpenAI id, and the map keeps `config.toml` consistent with the served model, D2). `buildEnv()` sets `OPENROUTER_API_KEY=<fake sentinel>`. `getProviders()` returns `openrouterProvider` (bearer) instead of the ChatGPT providers. Allowlist: `POST /api/v1/responses`, `GET /api/v1/models` (see §9).
...
- **Mapped slug (D2).** Codex has **no native model field in IronCurtain config** (no `codexModel`). It therefore must **never passthrough an unmapped OpenAI id**. The Codex slug is:
  `codexSlug = perAgent.codex ?? resolveMappedModel(DEFAULT_GLM_SLUG, modelMap) ?? DEFAULT_GLM_SLUG`. Applying `modelMap` to the default probe keeps `config.toml`'s `model` consistent with the model the MITM rewriter actually serves — the rewriter (D1) re-globs whatever Codex sends, so an omitted map step made `config.toml` disagree with the wire model whenever a glob matched the GLM default (Codex then budgeted its context window for a model it wasn't routed to). A non-matching map still falls back to `DEFAULT_GLM_SLUG`, so Codex never passthroughs an unmapped id. (Corrected from the original `perAgent.codex ?? DEFAULT_GLM_SLUG`, which excluded `modelMap` and introduced that drift.)
...
- **Codex:** `OPENROUTER_API_KEY` = fake key; no Codex OAuth env (`IRONCURTAIN_CODEX_ACCESS_TOKEN` absent). **B1/D2:** parse the generated `codex-config.toml` with `smol-toml` and assert top-level `model_provider === 'openrouter'` and `model === (perAgent.codex ?? resolveMappedModel(DEFAULT_GLM_SLUG, modelMap) ?? DEFAULT_GLM_SLUG)`. Cover both a matching map (`model ===` the mapped slug) and a non-matching map (`model === DEFAULT_GLM_SLUG`, never an unmapped OpenAI id).
