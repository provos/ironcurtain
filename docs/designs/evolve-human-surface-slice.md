...
Status: **Implemented & merged in #303 (2026-06-16).** Shipped as designed: three `human_gate` states (`preflight_review`/`human_escalation`/`final_review`), a `final_summary` agent state producing `final_report`, and a distinct `aborted` terminal, with `escalate`/`evaluator_blocked → human_escalation` and `complete → final_summary` retargets. The sections below are the **as-shipped record** — the design narrative is preserved, with as-shipped annotations marked **[SHIPPED]** where the implementation clarifies or deviates from the original plan (notably the §4.2/§5.2 `buildGateRequest` runtime change, which the design predicted would not be needed). All three maintainer decisions (§13) shipped exactly as decided. Note: the human-surface FSM described here is rooted at `preflight`; a later PR (#309) prepended a `provision`/`provision_review` stage as the new `initial:` state, so the live `workflow.yaml` now begins one state earlier — that stage is out of scope for this slice and does not change any human-surface edge documented below.

Original status (design phase, retained for history): _Design — ready to implement (3 maintainer decisions signed off 2026-06-16, §13). Independent critical review incorporated (2026-06-16): the human-gate machinery claims were verified correct; this revision owns the one intentional behavior change (`evaluator_blocked → human_escalation`, which retargets a previously-terminal edge and breaks two shipped integration tests — §12), strikes the WF004-violating gate `present:` rows from §4.5, specifies the §12 mock-harness `final_summary` branch, and resolves the budget-extension question (§8.3 / §13). All three residual maintainer decisions are now signed off (§13): `final_summary` = agent state; objective-change = conservative ABORT-to-restart; `evaluator_blocked` = reroute to `human_escalation`._
...
This slice is a **near-pure FSM/gate surface addition** (plus the three §1 edge retargets, one of which — `evaluator_blocked → human_escalation` — is a deliberate behavior change, not a topology-only superset). **[SHIPPED]** It was _almost_ engine/runtime-free as planned, but not entirely: the `evaluator_blocked` reroute also needed one small orchestrator-runtime change (`buildGateRequest`, §4.2 / §5.2 / §13 decision 3) to surface the deterministic verdict in the gate summary. No engine code, no determinism path, and no policy-engine behavior changed. Explicitly out of scope (no work here, no re-architecture):
...
> **[SHIPPED — deviation]** This "no workflow-runtime change is required" prediction was **not** quite borne out: PR #303 shipped one small, reviewed-as-necessary runtime change in `orchestrator.ts` (`buildGateRequest`). When `human_escalation` is reached via `evaluate ─evaluator_blocked`, the `evaluate` deterministic state resolves with `passed: false` and leaves `context.lastError` **null**, so the existing `lastError`-only summary logic surfaced no reason for the escalation. `buildGateRequest` was extended to fall back to the previous deterministic state's verdict (`context.lastDeterministicResult.verdict`) and suffix the gate summary with `(verdict: evaluator_blocked)` when there is no `lastError`. The human-gate _machinery_ was indeed already merged; the one new line of runtime is purely about making the deterministic escalation reason visible in the gate summary. The §12 Scenario C assertion `escalationGate.summary` contains `'evaluator_blocked'` is the proof. See the [SHIPPED] notes in §4.2 and §5.2 for the exact code.
...
> **[SHIPPED]** Both retargets above are the live edges (`orchestrator`/`evaluate` transitions, `workflow.yaml`). The `evaluator_blocked → human_escalation` edge required the one runtime change in the slice: `buildGateRequest` in `orchestrator.ts` now surfaces the deterministic `evaluator_blocked` verdict into the gate `summary` (the `evaluate` state leaves `context.lastError` null, so the prior summary logic showed no reason). See the §5 [SHIPPED] correction and §13 decision 3. The shipped `human_escalation` gate `description` is slightly richer than the verbatim block below — it states the objective-change conservative contract inline ("force a revision back to preflight for evaluator/spec repair that preserves existing node validity, or abort and start a fresh run if the objective must change"), per §13 decision 2.
...
Each gate, on entry, sets `phase: 'waiting_human'` and builds a `HumanGateRequest` whose `presentedArtifacts` is the set of `present:` names that resolve to an **existing directory under the run's artifact dir** (`<workspace>/.workflow/<name>/`, `orchestrator.ts:2552-2592` `buildGateRequest`). The `summary` is `"Waiting for human review at <gateName>"`, suffixed with the escalation reason when the gate was reached via a non-normal path.

> **[SHIPPED — correction]** The design assumed the suffix would always be `context.lastError`, "so a `human_escalation` reached via `evaluator_blocked` carries the engine's error string into the summary automatically, with no extra wiring." **That assumption was wrong** and was corrected during the #303 review: the `evaluate` deterministic state resolves an `evaluator_blocked` verdict with `passed: false` and leaves `context.lastError` **null**, so there was nothing for the old `lastError`-only suffix to surface. `buildGateRequest` was extended (the one runtime change in the slice, §2 / §13 decision 3) to fall back to the previous deterministic state's verdict and append `(verdict: evaluator_blocked)` when `lastError` is absent: `const errorContext = context?.lastError ? ` (error: …)`: previousDeterministicVerdict ?` (verdict: …)` : ''`. So the surfacing is automatic in the end — but via the verdict fallback, not via `lastError` as originally written.
...
- **Surfaces**: `summary` carries the escalation/eval-block reason suffix. **[SHIPPED]** For the `escalate` path this is `context.lastError`; for the `evaluator_blocked` path (where `lastError` is null — see the §5 [SHIPPED] correction) it is the `(verdict: evaluator_blocked)` fallback added to `buildGateRequest`. `present: [run_spec]` (the recommended honest set — see §4.5 note) — this is what shipped. The human can additionally inspect `.evolve_runs/main/database_data/nodes.json` and the in-flight `.evolve_runs/main/current/result.json` via `workflow inspect <baseDir>`; surfacing those through `present:` is blocked by WF004 (they are written by deterministic states, not agents — §4.5), so they are inspect-only, not gate-presented, unless a genuine agent producer is added (§13 "Lower-stakes decisions" — `human_escalation.present`).
...
- **FORCE_REVISION → orchestrator (run more rounds) accumulates orchestrator visits.** K extra rounds add ~`4K` orchestrator entries on top of the original `4N+1`. With `maxRounds: 200` and realistic N+K, this is far below the cap, so the backstop is not tripped by a single human-authorized extension. But to be safe and to keep the backstop's semantics ("safety ceiling on hub entries, not the round budget"), the FORCE*REVISION edges carry `resetVisitCounts: [orchestrator]` (and `[preflight, orchestrator]` for the preflight-bound edges) — clearing the accumulated visit count at the human-authorized boundary so a long sequence of human-driven extensions cannot spuriously trip the wedge guard. This is correct because the human pressing FORCE_REVISION \_is* the deliberate authorization the backstop exists to distinguish from a wedged agent.
...
> **[SHIPPED — gate green]** All three conditions held at merge (#303). The four real-Docker scenarios A/B/C/D shipped as **`test/workflow/evolve-human-surface.integration.test.ts`** (a dedicated new file, `describe.skipIf(!dockerReady)`, named tests `A:`/`B:`/`C:`/`D:`), driving the gates via `waitForGate` + `orchestrator.resolveGate`. The two pre-existing `evaluator_blocked → failed` tests (in `evolve-multi-round.integration.test.ts` and `evolve-single-round.integration.test.ts`) were updated to expect the `human_escalation` reroute. `workflow lint --strict` is clean, and a live end-to-end run drove both gates to `done` (3 nodes, intact lineage, a real `final_report`). The scenario specs below are the as-shipped record; one detail differs from the original §12.1 plan and is annotated there. (Note: the shipped scenarios begin at the `provision` state, asserting `run.states[0] === 'provision'`, because the later #309 provision stage is the FSM root in the merged tree — the human-surface assertions are otherwise exactly as designed.)

### 12.1 (a) Real-Docker integration test — extend the multi-round gate

A new test (or new `describe` blocks added to `test/workflow/evolve-multi-round.integration.test.ts`), gated `describe.skipIf(!dockerReady)` with the **same** `dockerReady` guard as the multi-round gate. It reuses that test's `TEST_HOME`/CA staging, `buildDockerSessionConfig`, `createInfra`/`destroyInfra`, `createDeps`, `onEvent` state collection, and the prompt-substring mock-routing of the four agent roles (multi-round §12.2).

> **[SHIPPED]** Shipped as a **dedicated new file** (`test/workflow/evolve-human-surface.integration.test.ts`) rather than `describe` blocks appended to the multi-round file, with a single parameterized `startScenario(scenario)` helper driving all four named tests (`A:`/`B:`/`C:`/`D:`). The mock router has the five expected prompt-substring branches plus a `provision` branch (`'provisioning the Python environment'`) for the #309 provision stage, and the `final_summary` branch matches `'The Evolve run has reached its round budget'` and writes a non-empty `final_report.md` into `.workflow/final_report/` (the §12.1 "fifth routing branch" requirement, satisfied). The new machinery is the **gate-driving helpers** (`test-helpers.ts:392-441`): `waitForGate(raiseGateMock, count, timeout)` (collects the raised `HumanGateRequest[]`), `waitForGateOrCompletion(orchestrator, workflowId, timeout)` (`'gate' | 'done'`), and `orchestrator.resolveGate(workflowId, { type, prompt? })` to inject the human event. `createDeps` must wire `raiseGate` to a `vi.fn()` so `waitForGate` can observe it.
...
> **[SHIPPED — done]** Both tests were updated in #303 to expect the `human_escalation` reroute (the gate is raised, then driven to a terminal) instead of the old `failed` terminal assertion. The no-double-record invariant ("a blocked round commits no node") is retained in both.
...
### Maintainer decisions — IMPLEMENTED as decided (signed off 2026-06-16, shipped in #303)

**[SHIPPED]** All three decisions below were signed off by the maintainer, each taking the recommended default, and all three **shipped exactly as decided** in PR #303. The rationale/tradeoffs are retained as the decision record. The live `workflow.yaml` reflects every choice: `final_summary` is an agent state (§4.3); `escalate`/`evaluator_blocked → human_escalation` and `complete → final_summary` are the shipped FSM edges (`orchestrator`/`evaluate` transitions, `workflow.yaml`); the `aborted` terminal exists; and the two pre-existing `evaluator_blocked → failed` integration tests were updated (§12). The objective-change conservative contract is stated verbatim in the shipped `human_escalation` gate description ("force a revision back to preflight for evaluator/spec repair that preserves existing node validity, or abort and start a fresh run if the objective must change") and the `preflight` prompt.

1. **`final_summary` as a pure agent state vs a deterministic helper (§6.4).** Both are buildable; this is a genuine cost/scope preference. _Recommended default: **agent state**_ (one YAML + one test, parent-design-faithful, no new `evolve_result.py` subcommand). **Tradeoff:** the agent version costs an LLM call per completion and produces a narrative report; a `deterministic`/`container` state running a templated `evolve_result.py final_summary` subcommand is cheaper and fully reproducible (testable without a model) but adds a bridge subcommand (mild scope creep against "single-file workflow scope") and yields a less useful report. **DECIDED (2026-06-16): agent state** — as assumed by the §4.3 / §6 embedded YAML. **[IMPLEMENTED in #303]** Shipped as a pure `agent` state (`final_summary`, `workflow.yaml`) with `inputs: [run_spec]`, `outputs: [final_report]`, and a single unconditional `to: final_review` transition; no `evolve_result.py final_summary` subcommand was added. The §12 Scenario A/D real-Docker tests confirm it materializes a non-empty `final_report.md` under `.workflow/final_report/`.
2. **`human_escalation → preflight` on objective change (§8.2).** Re-authoring `run_spec.yaml` does **not** clear `nodes.json`, so a FORCE*REVISION that *changes the objective\* leaves stale nodes counting toward `max_rounds` and being sampled as parents. **No safe engine "reset DB" operation was found** — re-authoring the spec does not clear the database, so stale nodes would corrupt both the round count and parent sampling. \_Recommended conservative contract: **"FORCE_REVISION preserves node validity; an objective change ⇒ ABORT and start a fresh run."\*** State this in the gate description and the preflight prompt. **Tradeoff:** the conservative contract is safe but means an operator who wants to keep the run alive across an objective pivot must abort and restart (losing the pause-resume convenience); the alternative — adding an engine DB-reset path on objective change — is real scope creep and needs its own design. **DECIDED (2026-06-16): adopt the conservative contract** — FORCE_REVISION preserves node validity; an objective change ⇒ ABORT and start a fresh run. No engine DB-reset path is added in this slice; state the contract in the `human_escalation` gate description and the `preflight` prompt. **[IMPLEMENTED in #303]** Shipped: no engine DB-reset path was added. The conservative contract is stated in the shipped `human_escalation` gate `description` ("force a revision back to preflight for evaluator/spec repair that preserves existing node validity, or abort and start a fresh run if the objective must change") and in the `preflight` prompt ("FORCE_REVISION preserves the validity of existing nodes; if the objective fundamentally changes, abort this run and start a fresh one instead of reusing stale nodes").
3. **`evaluator_blocked` routing — reroute to `human_escalation` vs keep hard-`failed` (§1, §4.2, §12).** This is the one **intentional behavior change** in the slice: it converts a previously-terminal `→ failed` outcome into a human-mediated gate, on the rationale that a single bad/crashed candidate should not hard-fail a multi-round search (a human decides retry / revise / abort). _Recommended default: **reroute** to `human_escalation`._ **Tradeoff:** rerouting makes a blocked candidate recoverable (and is the parent design's intent for evaluator-judgment stops) but is a UX change that **breaks two shipped integration tests** (§12) and changes operator-visible behavior; keeping hard-`failed` is the status-quo-preserving choice that needs no test churn but loses the recovery path the parent design wanted. (Note: this only moves the `evaluator_blocked` edge; `result_file_error`, sampler/engine crash, `needs_repair`, and the wedge backstop still hard-fail to `failed` — §4.2.) **DECIDED (2026-06-16): reroute to `human_escalation`** — the two shipped integration tests are updated per §12. **[IMPLEMENTED in #303]** Shipped: `evaluate ─evaluator_blocked → human_escalation` is the live edge (`evaluate` transitions, `workflow.yaml`); `result_file_error`, sampler/engine crash, `needs_repair`, and the `isRoundLimitReached` wedge backstop still hard-fail to `failed`. The two pre-existing integration tests were updated, and the new `evolve-human-surface.integration.test.ts` Scenario C asserts the reroute end-to-end (gate `stateName === 'human_escalation'`, `summary` contains `evaluator_blocked`, FORCE_REVISION recovery, and exactly N nodes with no spurious record). **This reroute also required the one runtime change in the slice** — the `buildGateRequest` change in `orchestrator.ts` (§4.2 / §5.2 [SHIPPED] notes), because `evaluate` resolves with `passed: false` and leaves `context.lastError` null, so the deterministic `evaluator_blocked` verdict had to be surfaced into the gate summary explicitly. This is a deviation from the §2 "no workflow-runtime change is required" claim; see the [SHIPPED] annotations there and in §5.
...
### Still-open verification (low risk)

- **Resume → gate re-surfacing (§8.1).** The doc asserts `workflow resume` re-surfaces the same `HumanGateRequest` for a still-pending gate. **[SHIPPED — partially exercised]** The four §12 scenarios drive each gate **in-process** via `waitForGate` + `orchestrator.resolveGate` (the gate-pause / `phase: 'waiting_human'` path is proven, including the FORCE_REVISION re-entry loops), but they do **not** exercise the cross-process `workflow resume <baseDir>` re-raise path — that remains identical to the already-shipped `vuln-discovery` gate-resume behavior and was not separately retested in #303.
