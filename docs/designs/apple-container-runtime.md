...
- **Phase 4 -- PTY path + config.** Direct host→container PTY connection (or `--publish-socket`); `containerRuntime` config field replacing the env var, config editor + first-start integration, `auto` detection.
