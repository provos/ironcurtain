...
Status: **Implemented & merged in #302 (2026-06-16).** The multi-round evolution loop shipped as designed — `orchestrator` agent hub, greedy sampling, cognition seed+retrieve, per-round analyzer, single durable `attach_analysis` write, and both determinism fixes — with one known drift corrected below (`settings.maxRounds` shipped as **200**, not 24). The sections below are the as-shipped record; both prior maintainer calls (§16) are resolved as implemented (`preflight_review` deferred here landed later in #303).
...
This is the exact, verified definition (it lints clean — see §3.4). `settings.maxRounds: 200` plus the `isRoundLimitReached` guard on the orchestrator is the terminal-safety backstop (§3.6). (As shipped in #302 the backstop is **200**, not the 24 originally drafted: `maxRounds` is a wedge ceiling counting orchestrator visits (~4 per round, plus the final `complete`), so it must sit well above `4 × run_spec.budget.max_rounds + 1`; 24 would have capped any legitimate run-spec `budget.max_rounds` above ~5. 200 keeps the safety net while never bounding a real round budget.)
...
settings:
  ...
  maxRounds: 200 # terminal-safety backstop (§3.6); the orchestrator's own complete verdict is the normal exit. Wedge ceiling counting orchestrator visits (~4/round), raised to 200 so it never caps a legitimate run-spec budget.max_rounds
...
- `settings.maxRounds: 200` is set, and the orchestrator's **first** transition is `guard: isRoundLimitReached → failed`. Listed first so it is evaluated before any verdict edge on each hub entry — if the cap is hit, the run terminates deterministically regardless of what verdict the agent emitted.
- **What the guard actually counts.** `isRoundLimitReached` compares `max(context.visitCounts) >= context.maxRounds` (`guards.ts:21-26`). `visitCounts` is incremented **only on agent-state entry** (`incrementVisitCount` is an entry action on agent states only — `machine-builder.ts:262`; deterministic states do not increment it). So the cap counts agent-state visits, and the most-visited agent state is the orchestrator (it re-enters once per routing decision). For a clean N-round run the orchestrator is entered `4N+1` times (the `design`/`evaluate`/`analyze`/`record` cycle per round — the round-1 `design` is the first entry — plus the final `complete`); at N=3 that is 13, matching §12. **As shipped (#302) the backstop is `maxRounds: 200`, not the 24 originally drafted.** Because the guard counts orchestrator visits (~4 per round + 1), a ceiling of 24 would trip at only ~5 legitimate rounds — capping a real run-spec `budget.max_rounds` instead of merely catching a wedge. 200 is raised so it never bounds a legitimate round budget while still terminating a hub that never emits `complete`. The value is a _safety ceiling on hub entries_, not the round budget itself (the round budget is `run_spec.yaml budget.max_rounds`, read by the orchestrator).
...
### 9.1 Item 1 — `embedding.py` process-randomized `hash()` → fix via `PYTHONHASHSEED=0` in the bridge subprocess env (IMPLEMENTED #302)

> **As shipped (#302):** the bridge defines `DETERMINISTIC_ENV = {**os.environ, "PYTHONHASHSEED": "0"}` (`evolve_result.py:16`) and passes it as `env=` on the single `_run_helper` subprocess seam (`evolve_result.py:163`), exactly as designed below. Every engine wrapper (`evolve-eval`/`evolve-db`/`evolve-cognition`) is launched through `_run_helper`, so the pinned seed reaches each child interpreter at startup. The end-to-end determinism gate compares `database_data/faiss/vector_store.pkl` byte-for-byte across two runs (`evolve-multi-round.integration.test.ts:503-505`) and passes.
...
### 9.2 Item 2 — `structures.py created_at = datetime.now()` → cosmetic; normalize in comparisons; upstream a caller-injectable field for true reproducibility (IMPLEMENTED #302)

> **As shipped (#302):** the engine is kept byte-verbatim (`created_at` is not injectable), and the determinism gate strips `created_at` per node before comparing two runs' `nodes.json` (`delete rest.created_at` in `stripCreatedAt`, `evolve-multi-round.integration.test.ts:239`), while comparing `vector_store.pkl` byte-for-byte as the timestamp-free witness (`:503-505`). The upstream `--created-at` flag remains deferred (§15 #5).
...
| File                                                     | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/workflow/workflows/evolve/workflow.yaml`            | **Replace** the linear FSM with the hub-and-spoke FSM (§3.3): `preflight → orchestrator` (agent hub) with `sample`, `researcher`, `evaluate`, `analyzer`, `analysis_record` as spokes; add `settings.maxRounds: 200` + the orchestrator's `isRoundLimitReached` backstop; agent spokes return via `to: orchestrator`, deterministic spokes via their verdict; extend `preflight` (author cognition seed + `max_rounds`/`cognition.source_mode seed`); rewire `researcher`/`evaluate` to per-round `current/` paths.                                                                                                                                                                                                                                                                                   |
...
**Signed off 2026-06-16 — IMPLEMENTED in #302:**

1. **Loop mechanism — the `orchestrator` agent hub (§3.1, §3.6). IMPLEMENTED.** _Decided: build the parent-design hub_, not a deterministic round controller. As shipped, the loop is driven by the `orchestrator` agent emitting `design`/`evaluate`/`analyze`/`record`/`complete`/`escalate`, with every specialist returning to the hub and a deterministic `isRoundLimitReached` backstop (WF006, `maxRounds: 200`) bounding a wedged agent. This made the slice parent-design-faithful from the first multi-round implementation. (Cost accepted: the §12 gate's mock feeds the orchestrator a scripted verdict sequence — spelled out in §12.2.)
2. **`preflight_review` — deferred for this slice (§11). IMPLEMENTED as deferral in #302; the gate itself LANDED LATER in #303.** _Decided: keep the human gate out of #302_ and ship it with the dedicated human-surface slice. In #302 the FSM terminated `complete → done` / `escalate → failed` with `approval.confirmed=true` set by `preflight` and helper-layer enforcement (`require_evolve_ready`, `run_state.py:288`). The deferred `preflight_review` (plus `provision_review`/`human_escalation`/`final_summary`/`final_review`/`aborted`) was subsequently added in **#303** (the human-surface slice); the current `workflow.yaml` on master reflects those #303 additions on top of this slice's hub.

**Still open at design time — RESOLVED as implemented:**

3. **Sampler choice — `greedy` for v1 (§4.1). RESOLVED: greedy shipped.** Greedy (deterministic, satisfies the parent-linkage gate, no island setup) is the shipped v1 sampler — `run_spec.yaml` sets `sampling-algorithm greedy`, `sample-n 1`. `ucb1`/`random` remain non-deterministic and deferred behind a seeding story (§15 #1); they did not ship in #302.
