...
- **No Network Egress (macOS, Docker backend):** The container runs on a Docker `--internal` bridge network with no default route and iptables DROP rules. A socat sidecar container bridges only the MCP and MITM proxy ports between the internal network and the host. The agent container cannot reach the host gateway or any host service directly — it can only reach the sidecar's forwarded ports.
- **No Network Egress (macOS, Apple container backend):** Each container is a dedicated lightweight VM (Virtualization.framework) on a per-bundle host-only vmnet network (`--internal`): internet egress is blocked at the network layer, verified by a fail-closed startup gate (proxies reachable at the gateway AND an egress probe must fail, else the session aborts). IronCurtain's proxies listen on 0.0.0.0 guarded by a connection-source predicate that admits only the bundle's subnet plus loopback.
...
**Residual risk (Apple container backend):** A host-only network routes agent→host traffic natively, so the agent VM can reach any host service bound to `0.0.0.0` (loopback-bound services are unreachable; IronCurtain's own proxies are subnet-guarded). This is broader port-level host exposure than the Docker sidecar's two forwarded ports, and is traded against a stronger Layer 1: a kernel exploit from the agent is contained by a hardware-virtualization boundary instead of shared-kernel namespaces. See docs/designs/apple-container-runtime.md.
...
- **Policy Constraints:** Even a jailbroken agent cannot bypass the Policy Engine. It can _ask_ to delete a file, but the policy will deny it. Structural invariants (protected paths, sandbox containment, unknown tool denial) are hardcoded and cannot be influenced by agent behavior.
...
- **Tool call arguments:** Theoretically, a credential could enter the LLM context (e.g., echoed in a tool response) and then be passed as an argument to another tool, appearing in the audit log. This is an indirect path that requires the MCP server to echo credentials in responses.
  ...
  **Residual risk:** Credentials echoed by MCP servers in tool _responses_ enter the LLM context and are not masked. Addressing this requires response filtering, which is a different mechanism and deferred.
