...
async function main(): Promise<void> {
  ...
  try {
    ...
    try {
      const active = await waitForActiveBundle(smokeHome, child, expectedConfigHash);
      activeBundle = active;
...
async function waitForActiveBundle(
  ...
  return poll('active Docker-workload lease', async () => {
    ...
    for (const sessionId of readdirSync(sessionsRoot)) {
      ...
      if (metadata.dockerWorkload.backend !== 'apple-container' || metadata.dockerWorkload.configHash !== configHash) {
...
async function waitForClosedLease(path: string): Promise<DockerWorkloadLease> {
  return poll('closed Docker-workload lease', () => {
...
async function waitForExit(child: ChildProcessWithoutNullStreams, timeoutMs: number): Promise<number | null> {
  ...
  return Promise.race([
    ...
    delay(timeoutMs).then(() => {
      throw new Error(`timed out waiting ${timeoutMs}ms for built CLI exit`);
    }),
  ]);
}

async function poll<T>(label: string, read: () => T | undefined | Promise<T | undefined>): Promise<T> {
