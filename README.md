...
- Node.js 22, 24, or 26 — the even LTS lines (required by `isolated-vm`; 24 and 26 install prebuilt binaries, Node 22 compiles from source at install and needs a C/C++ toolchain). Odd non-LTS lines (23, 25) run but are untested — `ironcurtain doctor` warns.
...
| **Node version errors**                 | Supported Node.js lines are 22, 24, and 26 — the even LTS releases (`isolated-vm`). 24 and 26 install prebuilt binaries; Node 22 compiles `isolated-vm` from source and needs a C/C++ toolchain. Odd non-LTS lines (23, 25) are untested — `ironcurtain doctor` flags them with a warning rather than a hard failure.                                                                                                                                                          |
