    ...
    try {
      ...
      const config: CreateDesktopRelayExposureOptions = {
        ...
        registry: {
          ...
          listenPort: 8443,
          upstream: { kind: 'tcp', host: 'host.docker.internal', port: registryListening.port },
        },
        package: {
          containerName: packageRelayName,
          relayIpv4Address: '172.31.44.4',
          listenPort: 8080,
          upstream: { kind: 'tcp', host: 'host.docker.internal', port: packageListening.port },
        },
    ...
    try {
      ...
      relay = await createDesktopRelayExposure(defaultExecFile, {
        ...
        registry: {
          ...
          listenPort: 8443,
          upstream: { kind: 'tcp', host: 'host.docker.internal', port: listening.port },
        },
        createOuterResource: directCreateAuthority,
      });
