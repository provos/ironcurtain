...
- `src/pipeline/` is offline tooling. Runtime code (`session/`, `sandbox/`, `trusted-process/`, `docker/`, `cron/`, `workflow/`, `memory/`, etc.) MUST NOT import from `pipeline/`. If a helper there is needed at runtime, it's misplaced — relocate it to the consumer's layer (often `trusted-process/`) or a leaf module (e.g., `observability/`).
