...
- **Agent skills (SKILL.md)** — drop SKILL.md packages under `~/.ironcurtain/skills/<name>/` to make purpose-specific guidance available to every Docker session; the merged set is staged so each agent's _native_ skill discovery picks it up. There is no cross-vendor standard path: Claude Code is pointed at the staging dir via `--add-dir <parent>` (it scans `<parent>/.claude/skills/<name>/SKILL.md`), Goose scans `~/.config/goose/skills/<name>/SKILL.md`. Each agent adapter declares its own `skillsContainerPath`; the Docker infrastructure always issues a dedicated **read-only** bind mount at that path (sibling of any conversation-state mount — nested bind mounts are unreliable on Docker Desktop / macOS, and read-only enforces the cached-stager + per-state filter assumptions). Workflows ship per-state skills via `<workflow-pkg>/skills/<name>/SKILL.md` and an optional `skills: [...]` field on agent states (omit = all workflow skills, present = filtered subset). Workflow definitions are now packaged as directories (`<name>/workflow.yaml`) so a workflow can ship its own `skills/` alongside the manifest. In workflow mode the staging dir is established once per bundle and re-staged in place on each state transition (the staging dir's parent inode is preserved across re-stages so Linux bind mounts stay valid). Persona skills (`~/.ironcurtain/personas/<name>/skills/`) apply to standalone sessions; they are intentionally inert in workflow mode where per-state `skills:` is the differentiation knob. See [WORKFLOWS.md](WORKFLOWS.md#skills).
...
- **Per-persona / per-job memory opt-in** — `PersonaDefinition.memory?: { enabled: boolean }` and `JobDefinition.memory?: { enabled: boolean }` let users disable the memory MCP server for individual personas / cron jobs. The global `userConfig.memory.enabled` kill switch (default on) still wins. `MEMORY_SERVER_NAME` was removed from the persona resolver's always-included set, so memory only flows through the same gate as every other server. The decision lives in `isMemoryEnabledFor` (`src/memory/memory-policy.ts`); the workflow orchestrator uses the loader-aware `isMemoryEnabledForPersonaName` (`src/persona/memory-gate.ts`) when deriving per-scope relays — the memory relay is now spawned only when at least one persona in that scope opts in, closing a workflow shared-container gap where the prompt advertised memory but the relay was absent. CLI onboarding (`persona create/edit`, `daemon add-job/edit-job`, `ironcurtain config -> Memory`) surfaces the toggle. See `docs/designs/per-persona-memory-optin.md`.
