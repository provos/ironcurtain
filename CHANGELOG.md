...
- **Agent skills (SKILL.md)** — drop SKILL.md packages under `~/.ironcurtain/skills/<name>/` to make purpose-specific guidance available to every Docker session; the merged set is bind-mounted read-only at `/home/codespace/.agents/skills/`, the cross-vendor common path used by Claude Code, Goose, and Codex. Workflows ship per-state skills via `<workflow-pkg>/skills/<name>/SKILL.md` and an optional `skills: [...]` field on agent states (omit = all workflow skills, present = filtered subset). Workflow definitions are now packaged as directories (`<name>/workflow.yaml`) so a workflow can ship its own `skills/` alongside the manifest. In workflow mode the bind mount is established once per bundle and re-staged in place on each state transition (the staging dir's parent inode is preserved across re-stages so Linux bind mounts stay valid). Persona skills (`~/.ironcurtain/personas/<name>/skills/`) apply to standalone sessions; they are intentionally inert in workflow mode where per-state `skills:` is the differentiation knob. See [WORKFLOWS.md](WORKFLOWS.md#skills).
