...
- `network-topology.ts` - Proxy-transport topology (`uds` | `tcp-sidecar` | `tcp-hostonly`). The `tcp-hostonly` topology (apple-container) creates a per-bundle host-only vmnet network from a 16-subnet pool and reaches host proxies at the gateway IP — no socat sidecar. Proxies listen on 0.0.0.0 guarded by `makeSourceAddressGuard` (the vmnet gateway interface doesn't exist until the first container attaches, so gateway binding is impossible). `IRONCURTAIN_CONTAINER_RUNTIME=apple-container` selects the backend until the config field lands; `checkHostOnlyConnectivity` in `docker-infrastructure.ts` is the fail-closed startup gate (gateway reachable AND egress blocked). PTY mode is not yet supported on this topology (fails fast).
