...
- `apple-container-manager.ts` - Apple `container` CLI wrapper implementing `ContainerRuntime` (macOS 26+ on Apple silicon, VM per container). `checkAppleContainerAvailable()` is the availability probe. Docker-only mechanisms (`extraHosts`, `restartPolicy`, `network: 'none'`, `connectNetwork`) throw instead of degrading silently. The only module allowed to spawn the `container` binary.
- `network-topology.ts` - Proxy-transport topology (`uds` | `tcp-sidecar` | `tcp-hostonly`). The `tcp-hostonly` topology (apple-container) creates a per-bundle host-only vmnet network from a 16-subnet pool and reaches host proxies at the gateway IP — no socat sidecar; the PTY connects host→container-IP directly. Proxies listen on 0.0.0.0 guarded by `makeSourceAddressGuard` (the vmnet gateway interface doesn't exist until the first container attaches, so gateway binding is impossible). Backend selection: `containerRuntime` config field (`auto` default — prefers apple-container when its probe passes; resolution memoized in `container-runtime.ts`), overridable via `IRONCURTAIN_CONTAINER_RUNTIME`. `checkHostOnlyConnectivity` in `docker-infrastructure.ts` is the fail-closed startup gate (gateway reachable AND egress blocked), and `writeHostOnlyAptProxyConfig` replaces the apt single-file bind mount (apple container's virtiofs shares directories only); both are shared by batch and PTY modes.
