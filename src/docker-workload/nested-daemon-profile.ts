...
export interface NestedDaemonSeccompProfile {
  readonly defaultAction: string;
  readonly defaultErrnoRet: number;
  readonly archMap: readonly { readonly architecture: string; readonly subArchitectures: readonly string[] | null }[];
  readonly syscalls: readonly {
    readonly names: readonly string[];
    readonly action: string;
    readonly errnoRet?: number;
    readonly comment?: string;
    readonly args?: readonly { readonly index: number; readonly value: number; readonly op: string }[];
    readonly includes?: SeccompFilter;
    readonly excludes?: SeccompFilter;
  }[];
}

// One definition replaces the independent JSON artifact and its byte digest.
// It includes the full architecture, capability-conditional and argument rules.
const PROFILE = {
  ...
  syscalls: [
    ...
    {
      names: ['get_mempolicy', 'mbind', 'set_mempolicy', 'set_mempolicy_home_node'],
      action: 'SCMP_ACT_ALLOW',
      includes: {
        caps: ['CAP_SYS_NICE'],
      },
    ...
    },
    {
      ...
      action: 'SCMP_ACT_ALLOW',
      comment:
        'IronCurtain P2: denial-proven nested namespace lifecycle; outer capabilities and namespaces remain unchanged',
    },
    {
      names: ['mount'],
      action: 'SCMP_ACT_ALLOW',
      comment: 'IronCurtain P2: denial-proven RootlessKit mount propagation inside the child user namespace',
    },
    {
      names: ['pivot_root'],
      action: 'SCMP_ACT_ALLOW',
      comment: 'IronCurtain P2: denial-proven Moby image-unpack root switch inside the nested mount namespace',
    },
    {
      names: ['setns'],
      action: 'SCMP_ACT_ALLOW',
      comment: 'IronCurtain P2: denial-proven RootlessKit parent re-entry into its nested user namespace',
    },
    {
      names: ['unshare'],
      action: 'SCMP_ACT_ALLOW',
      comment: 'IronCurtain P2: denial-proven rootless user-namespace prerequisite; no capabilities added',
    },
    {
      names: ['umount2'],
      action: 'SCMP_ACT_ALLOW',
...
} satisfies NestedDaemonSeccompProfile;
