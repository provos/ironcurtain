...
2. **Sandbox containment** -- the sandbox directory is \`${config.allowedDirectory ?? '(configured at runtime)'}\`. Any tool call where ALL sandbox-safe path-role arguments are within the sandbox is automatically allowed by the engine, and those roles are structurally resolved (compiled rules are not evaluated for them). For filesystem multi-path tools (e.g. \`read_multiple_files\`), sandbox paths are also discharged structurally on a per-element basis even when the array mixes sandbox paths with paths in another permitted directory. Do NOT generate \`paths.within\` rules for filesystem tools targeting the sandbox -- that behavior is structural.
