 ...
 * Conditional role specs are surfaced with their discriminator argument names
 * and valid values; static role arrays render as flat lists.
 */
function formatAnnotationForPrompt(annotation: StoredToolAnnotation): string {
  const argsDesc = Object.entries(annotation.args)
    .map(([name, spec]) => {
...
export function buildGeneratorSystemPrompt(
  ...
  const annotationsSummary = annotations.map(formatAnnotationForPrompt).join('\n');
...
export async function generateScenarios(
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const argsSuperRefine = buildScenarioArgsSuperRefine(buildToolArgNamesMap(batch));
...
export async function repairScenarios(
  ...
  const argsSuperRefine = buildScenarioArgsSuperRefine(buildToolArgNamesMap(annotations));
  const schema = buildGeneratorResponseSchema(serverNames, toolNames, argsSuperRefine);
