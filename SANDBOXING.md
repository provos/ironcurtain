...
**Escape hatch -- `IRONCURTAIN_MITM_ALLOW_ALL_HOSTS`:** Setting `IRONCURTAIN_MITM_ALLOW_ALL_HOSTS=1` on the host process makes the MITM proxy treat *every* unknown host as a passthrough tunnel, bypassing the CONNECT allowlist entirely. This is a deliberate egress-filter bypass intended for build flows that fetch from many hosts (e.g. Bazel pulling from arbitrary mirror endpoints) where `add_proxy_domain` round-trips are impractical. **Only use in trusted environments.** The proxy logs a `WARN` on startup when this is set. Provider/registry traffic still flows through the normal MITM (TLS termination, credential swap, endpoint filtering) -- only unknown hosts are affected.
